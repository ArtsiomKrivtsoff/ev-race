...
import { upsertRoundWinnerForRound } from '../_shared/upsert-round-winner.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    for (let rn = 1; rn <= throughRound; rn++) {
      const winner = await upsertRoundWinnerForRound(supabase, rn, true);
      results.push({
        round_number: rn,
        winner: winner?.operator || null,
        stations: winner?.stations ?? 0,
      });
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        winners_upserted: results.filter((r) => r.winner).length,
        results,
      }),
